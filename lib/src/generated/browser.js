"use strict";
// This code was inserted by the 'WebServer SYNC 1.5.0' library. It's here to enable features
// like browser auto-reloading or showing messages. It does this by
// communicating with the WebServer SYNC 1.5.0 server via a websocket.
//
// Configuration dependent values that are passed/interpolated by WebServer SYNC 1.5.0
// server.
...
socket.addEventListener("open", () => {
    ...
    socket.addEventListener("close", () => {
        console.log("WebServer SYNC 1.5.0 server closed WS connection: trying to reconnect...");
        tryReconnect();
    });
...
function onMessage(event) {
    if (typeof event.data !== 'string') {
        throw new Error("unexpected WS message from WebServer SYNC 1.5.0");
    }
    ...
    switch (command) {
        case "reload":
            console.log("Received reload request from WebServer SYNC 1.5.0 server: reloading page...");
            location.reload();
            break;
        case "message":
            showMessage(payload);
            break;
        default:
            throw new Error("unexpected WS command from WebServer SYNC 1.5.0");
    }
}
